import { Image, Modal, StyleSheet, TouchableHighlightProps, TouchableOpacity, View } from 'react-native';
...
export const PersonListItem = ({
  ...
  const { selectedStaff, setSelectedStaff, updateStaffAccess, selectedCourse, removeStaffFromCourse} = useCourses()
...
subtitle={`${subtitle} - ${accessTranslationMap[staff?.access as keyof typeof accessTranslationMap] || staff?.access}`}
      trailingItem={
      ...
      onPress={()=>{
        if(subtitle != 'Titolare' && subtitle != 'Owner'){
