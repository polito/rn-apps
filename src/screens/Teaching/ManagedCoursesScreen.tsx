...
export const ManagedCoursesScreen = () => {
  ...
  const { fakeCourses, fakeExams, managedCourses, setSelectedCourse } = useCourses(); // Usa il hook per ottenere i dati
  const navigation = useNavigation();

  // 📌 Raggruppa i corsi per periodo
  const coursesByPeriod = managedCourses.reduce((acc, course) => {
    (acc[course.period] = acc[course.period] || []).push(course);
    return acc;
  }, {} as Record<string, typeof managedCourses>);
