...
| `students/assets/translations/it.json`     | Stringhe i18n italiano — **sempre aggiornare** in coppia con en.json            |
...
Due elementi con `accessibilityRole="button"` annidati fanno deviare il focus di VoiceOver su iOS: alcune card navigano invece di attivare il preferito, altre attivano il preferito invece di navigare. Mantenere **un solo** ruolo button sul contenitore esterno navigabile; le azioni interne (es. `IconButton` preferito) mantengono il proprio ruolo e `accessibilityLabel`, ma non devono stare dentro un altro elemento che dichiara anche `button`.

```tsx
// SBAGLIATO — Row + TouchableCard entrambi button; focus VoiceOver inaffidabile
...
// CORRETTO — role button solo su TouchableCard; IconButton è controllo separato
<TouchableCard accessibilityRole="button" onPress={navigate}>
  <Row>
    <IconButton
      accessibilityLabel={
...
> **Non reinventare `Checkbox`.** Il componente `students/src/core/components/Checkbox.tsx` gestisce già `accessibilityRole="checkbox"`, `accessibilityState`, e `hitSlop`. Non reimplementare manualmente.
...
`AccessibleFlatList` (`students/src/core/components/AccessibleFlatList.tsx`) aggiunge automaticamente annuncio del conteggio e label di posizione per ogni item. Usarlo sempre quando la lista è esposta agli screen reader.
...
```tsx
<View accessibilityViewIsModal={true}>{/* contenuto del modal */}</View>
```
...
> **Non usare `accessibilityLiveRegion` da solo per i loading.** Abbinarlo sempre a `announceLoading` da `useAccessibility` — fornisce il meccanismo più robusto su entrambe le piattaforme.
