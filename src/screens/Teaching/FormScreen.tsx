...
export const FormScreen = () => {
  ...
  const { selectedCourse, addNoticeToCourse, addMaterialToCourse , addLesson} = useCourses();
  ...
  const handlePublish = () => {
    console.log({ formType, title, description, startDate, endDate, selectedDirectory});
    ...
    if (formType === 'materiale' && selectedCourse?.id != null && selectedDirectory && title) {
        // Trova l'ID massimo tra tutti i file del corso
        const allFiles = selectedCourse.directories.flatMap(dir => dir.files);
        const lastFileId = allFiles.length > 0 ? Math.max(...allFiles.map(f => f.id)) : 0;
        ...
        addMaterialToCourse(selectedCourse.id, Number(selectedDirectory), newMaterial);
      }

      if (formType === 'lezione' && selectedCourse?.id != null && title && startDate) {
        const lastLessonId =
  ...
  <Row justify="space-between">
    ...
    <TouchableOpacity
      ...
      style={[ styles.button , { width: 150, height: 50, justifyContent: 'center', alignItems: 'center' }] }
    >
  ...
  <Text style={{ marginTop: 20, fontSize: 16, marginLeft : 20 }}>Data inizio: {formatDate(startDate)}</Text>  
  <Text style={{ marginTop: 10, fontSize: 16 , marginLeft : 20}}>Data fine: {formatDate(endDate)}</Text>   
</Card>
                ...
                {showStartPicker && (
                  <DateTimePicker
                    ...
                    onChange={(event, date) => handleDateChange(event, date, setShowStartPicker, setStartDate)}
                  />
                )}
                {showEndPicker && (
                  <DateTimePicker
                    ...
                    mode="datetime"
                    display={Platform.OS === 'ios' ? 'spinner' : 'default'}
                    onChange={(event, date) => handleDateChange(event, date, setShowEndPicker, setEndDate)}
                  />
                )}
              </>
            )}
            ...
            {formType === 'materiale' && (
              <>
                ...
                <Select
                    ...
                    options={selectedCourse?.directories?.map((dir) => ({
  ...
  <>
    ...
    <Card  style={{
                    ...
                    <IconButton
                        ...
                        size = {40}
                    />
                    <Text style ={{ textAlign: 'center' }}>Collega videolezione</Text>
                    </Card>
